    ...
    def pretrain_loss(self, pre_gen_out, HR_data):
        with tf.name_scope('loss_function'):
            with tf.variable_scope('pixel-wise_loss'):
                pre_gen_loss = tf.reduce_mean(tf.reduce_sum(tf.abs(pre_gen_out - HR_data), axis=[1, 2, 3]))
    ...
    def pretrain_optimizer(FLAGS, global_iter, pre_gen_loss):
        ...
        with tf.name_scope('optimizer'):
            with tf.variable_scope('optimizer_generator'):
                pre_gen_var = tf.compat.v1.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='generator')
                pre_gen_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate).minimize(loss=pre_gen_loss,
