...
import glob
from lib.data import load_train_dataset
...
def train_pretrain_generator(FLAGS, logflag):
    ...
    with tf.Session(config=config) as sess:
        ...
        lr_hr_ds, n_data = load_train_dataset(os.path.join(FLAGS.LR_data), os.path.join(FLAGS.HR_data), '.png', FLAGS.batch_size)
        ...
        for epoch in range(num_epoch):
            ...
            for iteration in range(num_batch_in_train):
                ...
                if current_iter % FLAGS.train_ckpt_save_freq == 0:
                    saver.save(sess, os.path.join(FLAGS.pre_train_checkpoint_dir), global_step=current_iter)

        writer.close()
        mox.file.copy_parallel(FLAGS.pre_train_checkpoint_dir, "s3://esrgan-div2k/checkpoint/pre_train_checkpoint")
