...
def set_flags():
    ...
    # About data
    Flags.DEFINE_string('data_url', 's3://esrgan-div2k/npz', 'The npz data dir')
    Flags.DEFINE_string('HR_data', './tmp/data/npz/DIV2K/HR_data', 'HR data dict')
    Flags.DEFINE_string('LR_data', './tmp/data/npz/DIV2K/LR_data', 'LR data dict')
    ...
    Flags.DEFINE_string('train_url', './tmp/data/npz/train_result', 'output directory during training')
    ...
    Flags.DEFINE_integer('num_repeat_RRDB', 23, 'The number of repeats of RRDB blocks')
    ...
    Flags.DEFINE_string('perceptual_loss', 'pixel-wise', 'the part of loss function. "VGG19" or "pixel-wise"')
    Flags.DEFINE_string('gan_loss_type', 'RaGAN', 'the type of GAN loss functions. "RaGAN or GAN"')

    # About training
    Flags.DEFINE_integer('num_iter', 2000, 'The number of iterations')
    ...
    Flags.DEFINE_string('pre_train_checkpoint_dir', './tmp/data/npz/pre_train_checkpoint', 'pre-train checkpoint directory')
    Flags.DEFINE_string('checkpoint_dir', './tmp/data/npz/checkpoint_dir', 'checkpoint directory')
    Flags.DEFINE_string('logdir', './tmp/data/npz/log_dir', 'log directory')
    Flags.DEFINE_string('host_pre_train', "s3://esrgan-div2k/data/pre_train_checkpoint", 'pre train checkpoint')
...
def main():
    ...
    with tf.Session(config=config) as sess:
        ...
        pre_saver.restore(sess, tf.train.latest_checkpoint(FLAGS.host_pre_train))
        ...
        writer.close()
        mox.file.copy_parallel(FLAGS.checkpoint_dir, "s3://esrgan-div2k/data/checkpoint")
...
if __name__ == '__main__':
    ...
    main()
    mox.file.copy_parallel("/var/log/npu/slog", "s3://esrgan-div2k/host_log")
